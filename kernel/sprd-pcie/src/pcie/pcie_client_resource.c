...
static const char *change_msg[EPC_LINKUP_MSG + 1] = {
	"rc scanned", "rc removing", "epc unlink", "epc linkup"
};

static const char *state_msg[SPRD_PCIE_REMOVED + 1] = {
	"wait first ready", "wait sacanned", "scanned", "wait remove", "removed"
};
...
static void sprd_pcie_resource_first_ready_notify(struct sprd_pcie_res *res)
{
	 ...
	 */
	res->sipa_res = pcie_sipa_res_create(res->dst, SIPA_RM_RES_PROD_PCIE_EP,
...
int sprd_pcie_resource_client_init(u32 dst, u32 ep_fun)
{
	...
	sprd_pci_epf_register_irq_handler_ex(res->ep_fun, PCIE_DBEL_EP_SCANNED,
					     PCIE_DBEL_EP_REMOVING,
					     pcie_resource_client_irq_handler,
					     res);
	sprd_pci_epf_register_notify(res->ep_fun,
				     pcie_resource_client_epf_notify, res);
...
int sprd_pcie_resource_trash(u32 dst)
{
	...
	sprd_pci_epf_unregister_irq_handler_ex(
		res->ep_fun, PCIE_DBEL_EP_SCANNED, PCIE_DBEL_EP_REMOVING);
...
int sprd_pcie_wait_resource(u32 dst, int timeout)
{
	...
	if (timeout < 0) {
		wait = wait_event_interruptible(
			res->wait_pcie_ready, res->state == SPRD_PCIE_SCANNED);
		ret = wait;
	} else {
		/*
		 * timeout must add 1s,
...
int sprd_pcie_request_resource(u32 dst)
{
	...
	case SPRD_PCIE_SCANNED:
		...
		ret = sprd_pci_epf_raise_irq(res->ep_fun, PCIE_MSI_REQUEST_RES);
		break;
...
int sprd_pcie_release_resource(u32 dst)
{
	...
	case SPRD_PCIE_SCANNED:
		...
		ret = sprd_pci_epf_raise_irq(res->ep_fun, PCIE_MSI_RELEASE_RES);
		break;
...
int sprd_register_pcie_resource_first_ready(u32 dst, void (*notify)(void *p),
					    void *data)
{
	struct sprd_pcie_res *res;
