...
static void pcie_sipa_res_wait_res_work_fn(struct work_struct *work)
{
	...
	struct pcie_sipa_res_prod *res =
		container_of(work, struct pcie_sipa_res_prod, wait_work);
	...
	sipa_rm_notify_completion(SIPA_RM_EVT_GRANTED, res->prod_id);
}
...
static void pcie_sipa_res_create_rm_work_fn(struct work_struct *work)
{
	...
	struct pcie_sipa_res_prod *res = container_of(
		to_delayed_work(work), struct pcie_sipa_res_prod, rm_work);
