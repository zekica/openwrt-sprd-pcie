...
static int sipa_init_tft_cmn_fifo_reg_addr(struct sipa_core *ipa)
{
	ipa->tft_reg_mapped = sprd_ep_ipa_map(PCIE_IPA_TYPE_TFT_REG,
					      ipa->tft_res->start,
					      resource_size(ipa->tft_res));
	ipa->tft_virt_reg_addr = devm_ioremap_nocache(
		ipa->dev, (resource_size_t)ipa->tft_reg_mapped,
...
static int sipa_init_cmn_fifo_reg_addr(struct sipa_core *ipa)
{
	...
	ipa->virt_reg_addr = devm_ioremap_nocache(
		ipa->dev, (resource_size_t)ipa->reg_mapped,
...
static int conn_thread(void *data)
{
	...
	if (ret != 0) {
		dev_err(ipa->dev,
			"sipa_delegator failed to open dst %d channel %d\n",
	...
	while (!kthread_should_stop()) {
		...
		case SMSG_TYPE_DONE:
			...
			dev_info(ipa->dev,
				 "remote ipa ready reg_mapped = 0x%llx\n",
		...
		if (ret) {
			dev_info(ipa->dev,
				 "unknown msg in conn_thrd: %d, %d, %d\n",
				 mrecv.type, mrecv.flag, mrecv.value);
			ret = 0;
		}
	}
