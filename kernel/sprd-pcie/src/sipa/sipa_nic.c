...
#define SIPA_CP_SRC                                                           \
	((1 << SIPA_TERM_CP0) | (1 << SIPA_TERM_CP1) | (1 << SIPA_TERM_VCP) | \
	 (1 << 0x19) | (1 << 0x18))
...
int sipa_nic_open(enum sipa_term_type src, int netid, sipa_notify_cb cb,
		  void *priv)
{
...
int sipa_nic_tx(enum sipa_nic_id nic_id, enum sipa_term_type dst, int netid,
		struct sk_buff *skb)
{
	int ret;
	struct sipa_core *ctrl = sipa_get_ctrl_pointer();
...
int sipa_nic_trigger_flow_ctrl_work(enum sipa_nic_id nic_id, int err)
{
	...
	case -EAGAIN:
		...
		break;
	default:
		dev_warn(ctrl->dev, "don't have this flow ctrl err type\n");
		break;
	}
