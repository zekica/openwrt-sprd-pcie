...
static int put_recv_array_node(struct sipa_skb_array *p, struct sk_buff *skb,
			       u64 *dma_addr)
{
	u32 pos;
...
static int get_recv_array_node(struct sipa_skb_array *p, struct sk_buff **skb,
			       u64 *dma_addr)
{
	u32 pos;
...
{
	p->array = kzalloc(sizeof(*p->array) * depth, GFP_KERNEL);
	if (!p->array)
		return -ENOMEM;
	p->rp = 0;
	p->wp = 0;
					...
					u32 cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		dma_addr = (u64)dma_map_single(
			ctrl->pci_dev, skb->head,
			SIPA_RECV_BUF_LEN + skb_headroom(skb), DMA_FROM_DEVICE);
		if (dma_mapping_error(ctrl->pci_dev, (dma_addr_t)dma_addr)) {
			dev_kfree_skb_any(skb);
			dev_err(ctrl->dev, "prepare free node dma map err\n");
			fail_cnt++;
			break;
		}
		...
		if (!node) {
			dma_unmap_single(ctrl->pci_dev, dma_addr,
					 ...
					 DMA_FROM_DEVICE);
			dev_kfree_skb_any(skb);
			dev_err(ctrl->dev, "get node fail index = %d\n", i);
			fail_cnt++;
			break;
		}
		...
		if (dma_addr == 0 || node->address == 0)
			pr_info("cnt = %d, i = %d, dma_addr 0x%llx, node->address 0x%llx\n",
				cnt, i, dma_addr,
				(long long unsigned int)node->address);
		put_recv_array_node(&receiver->recv_array, skb, &dma_addr);
		success_cnt++;
	}
	if (fail_cnt)
		dev_err(ctrl->dev, "fail_cnt = %d success_cnt = %d\n", fail_cnt,
			success_cnt);
}
...
static void fill_free_fifo(struct sipa_skb_receiver *receiver, u32 cnt)
{
	...
	if (success_cnt) {
		ctrl->hal_ops.set_rx_fifo_wr(ctrl->pci_dev, cmn->fifo_id,
					     ctrl->cmn_fifo_cfg, success_cnt);
		if (atomic_read(&receiver->need_fill_cnt) > 0)
			atomic_sub(success_cnt, &receiver->need_fill_cnt);
	}

	if (fail_cnt)
		dev_err(ctrl->dev, "fill free fifo fail_cnt = %d\n", fail_cnt);
}
				    ...
				    u32 data)
{
	...
	if (evt & SIPA_RECV_WARN_EVT) {
		dev_dbg(ctrl->dev, "sipa maybe poor resources evt = 0x%x\n",
			evt);
...
static void sipa_free_recv_skb(struct sipa_skb_receiver *receiver)
{
	...
	while (!get_recv_array_node(&receiver->recv_array, &recv_skb, &addr)) {
		dma_unmap_single(ctrl->pci_dev,
				 (dma_addr_t)(addr - ctrl->pcie_mem_offset),
...
static void sipa_receiver_init(struct sipa_skb_receiver *receiver, u32 rsvd)
{
	...
	dev_info(ctrl->dev, "fifo_id = %d rx_fifo depth = 0x%x\n",
		 receiver->ep->recv_fifo->fifo_id,
			     ...
			     struct sipa_skb_receiver **receiver_pp)
{
	...
	receiver->run = 1;
	receiver->fill_thread =
		kthread_create(fill_recv_thread, receiver, "sipa-fill");
