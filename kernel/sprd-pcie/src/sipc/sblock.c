	...
	*(poolhd_op->tx_rd_p) = *(poolhd_op->tx_wt_p);
	for (i = 0, j = 0; i < poolhd_op->tx_count; i++) {
		if (ring->txrecord[i] == SBLOCK_BLK_STATE_DONE) {
			ring->p_txblks[j].addr =
				i * sblock->txblksz + poolhd_op->tx_addr;
	...
	*(poolhd_op->rx_wt_p) = *(poolhd_op->rx_rd_p);
	for (i = 0, j = 0; i < poolhd_op->rx_count; i++) {
		if (ring->rxrecord[i] == SBLOCK_BLK_STATE_DONE) {
			ring->p_rxblks[j].addr =
				i * sblock->rxblksz + poolhd_op->rx_addr;
...
static int sblock_host_init(struct smsg_ipc *sipc, struct sblock_mgr *sblock,
			    u32 txblocknum, u32 txblocksize, u32 rxblocknum,
			    u32 rxblocksize)
{
	...
	pr_debug("%s: channel %d-%d, txblocksize=%#x, rxblocksize=%#x!\n",
		 __func__, sblock->dst, sblock->channel, txblocksize,
		 rxblocksize);

	pr_debug("%s: channel %d-%d, txblocknum=%#x, rxblocknum=%#x!\n",
		 __func__, sblock->dst, sblock->channel, txblocknum,
		 rxblocknum);
	...
	sblock->dst_smem_addr =
		sblock->smem_addr - sipc->smem_base + sipc->dst_smem_base;
	...
	pr_debug("%s: channel %d-%d, offset = 0x%lx!\n", __func__, sblock->dst,
		 sblock->channel, (unsigned long)offset);

	sblock->smem_virt = (void *)sblock->smem_addr;
	/* alloc ring */
	...
	ringhd->txblk_blks = ringhd->txblk_addr + txblocknum * txblocksize +
	...
	ringhd->rxblk_blks =
		ringhd->txblk_blks + txblocknum * sizeof(struct sblock_blks);
	...
	poolhd->txblk_blks =
		ringhd->rxblk_blks + rxblocknum * sizeof(struct sblock_blks);
	...
	poolhd->rxblk_blks =
		poolhd->txblk_blks + txblocknum * sizeof(struct sblock_blks);
	...
	sblock->ring->txblk_virt =
		sblock->smem_virt +
		(ringhd->txblk_addr - sblock->stored_smem_addr);
	sblock->ring->r_txblks = sblock->smem_virt + (ringhd->txblk_blks -
						      sblock->stored_smem_addr);
	sblock->ring->rxblk_virt =
		sblock->smem_virt +
		(ringhd->rxblk_addr - sblock->stored_smem_addr);
	sblock->ring->r_rxblks = sblock->smem_virt + (ringhd->rxblk_blks -
						      sblock->stored_smem_addr);
	sblock->ring->p_txblks = sblock->smem_virt + (poolhd->txblk_blks -
						      sblock->stored_smem_addr);
	sblock->ring->p_rxblks = sblock->smem_virt + (poolhd->rxblk_blks -
						      sblock->stored_smem_addr);
...
static int sblock_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		pr_debug(
			"sblock thread recv msg: dst=%d, channel=%d, type=%d, flag=0x%04x, value=0x%08x\n",
			sblock->dst, sblock->channel, mrecv.type, mrecv.flag,
			mrecv.value);
		...
		if (rval) {
			pr_info("non-handled sblock msg: %d-%d, %d, %d, %d\n",
				sblock->dst, sblock->channel, mrecv.type,
...
static int sblock_mgr_create(uint8_t dst, uint8_t channel, int pre_cfg,
			     ...
			     struct sblock_mgr **sb_mgr)
{
	...
	if (!sipc->client) {
		ret = sblock_host_init(sipc, sblock, tx_blk_num, tx_blk_sz,
...
int sblock_create_ex(u8 dst, u8 channel, u32 txblocknum, u32 txblocksize,
		     ...
		     void (*handler)(int event, void *data), void *data)
{
	...
	result = sblock_mgr_create(dst, channel, 0, txblocknum, txblocksize,
				   rxblocknum, rxblocksize, &sblock);
	if (!result) {
		...
		if (IS_ERR(sblock->thread)) {
			...
			sipc = smsg_ipcs[sblock->dst];
			if (!sipc->client) {
				smem_free(dst, sblock->smem_addr,
					  sblock->smem_size);
				kfree(sblock->ring->txrecord);
				kfree(sblock->ring->rxrecord);
			}
		...
		if ((handler != NULL) && (data != NULL)) {
			result = sblock_register_notifier(dst, channel, handler,
							  data);
			if (result < 0) {
...
int sblock_create(u8 dst, u8 channel, u32 txblocknum, u32 txblocksize,
		  u32 rxblocknum, u32 rxblocksize)
{
		       ...
		       u32 rx_blk_num, u32 rx_blk_sz)
{
	...
	result = sblock_mgr_create(dst, channel, 1, tx_blk_num, tx_blk_sz,
				   rx_blk_num, rx_blk_sz, &sblock);
	if (!result) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 9, 0))
		struct sched_param param = { .sched_priority = 11 };
#endif
...
int sblock_pcfg_open(uint8_t dest, uint8_t channel,
		     void (*notifier)(int event, void *client), void *client)
{
	struct sblock_mgr *sblock;
	uint8_t idx;
	int ret;
	...
	if (sblock->thread) {
		pr_err("%s: SBLOCK %u/%u already open", __func__,
	...
	ret = 0;
	sblock->thread = kthread_create(sblock_thread, sblock, "sblock-%d-%d",
					dest, channel);
	if (IS_ERR(sblock->thread)) {
...
int sblock_get_smem_cp_addr(uint8_t dest, uint8_t channel, uint32_t *paddr)
{
	struct sblock_mgr *sblock;
	uint8_t idx;
...
int sblock_get(u8 dst, u8 channel, struct sblock *blk, int timeout)
{
	...
	if (*(poolhd_op->tx_rd_p) != *(poolhd_op->tx_wt_p) &&
		...
		txpos = sblock_get_ringpos(*(poolhd_op->tx_rd_p),
					   poolhd_op->tx_count);
		blk->addr = sblock->smem_virt + (ring->p_txblks[txpos].addr -
						 sblock->stored_smem_addr);
...
static int sblock_send_ex(u8 dst, u8 channel, struct sblock *blk, bool yell)
{
	struct sblock_mgr *sblock;
	struct sblock_ring *ring;
	struct sblock_ring_header_op *ringhd_op;
	...
	pr_debug("sblock_send: channel=%d, wrptr=%d, txpos=%d, addr=%x\n",
		 channel, *(ringhd_op->tx_wt_p), txpos,
		 ring->r_txblks[txpos].addr);
...
int sblock_receive(u8 dst, u8 channel, struct sblock *blk, int timeout)
{
	struct sblock_mgr *sblock;
	struct sblock_ring *ring;
	struct sblock_ring_header_op *ringhd_op;
	...
	pr_debug("%s: channel=%d, wrptr=%d, rdptr=%d", __func__, channel,
	...
	if (no_data) {
		if (timeout == 0) {
			...
			/* wait forever */
			rval = wait_event_interruptible(
				ring->recvwait, sblock_has_data(sblock, false));
			if (rval < 0)
	...
	if (*(ringhd_op->rx_wt_p) != *(ringhd_op->rx_rd_p) &&
		...
		blk->addr = ring->r_rxblks[rxpos].addr -
			    sblock->stored_smem_addr + sblock->smem_virt;
...
unsigned int sblock_poll_wait(u8 dst, u8 channel, struct file *filp,
			      poll_table *wait)
{
	struct sblock_mgr *sblock;
	struct sblock_ring *ring;
...
static int sblock_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		...
		/* must request resource before read or write share memory */
		if (sipc_smem_request_resource(sipc->sipc_pms, sipc->dst,
					       1000) < 0)
			continue;
		...
		for (j = 0; j < SMSG_VALID_CH_NR; j++) {
			...
			seq_printf(
				...
				sblock->smem_size, sblock->txblksz,
				sblock->rxblksz);
			...
			seq_printf(
				m,
				"sblock ring: txblk_virt :0x%lx, rxblk_virt :0x%lx, poll_mask=0x%x\n",
				(unsigned long)ring->txblk_virt,
				(unsigned long)ring->rxblk_virt,
				ring->poll_mask);
			seq_printf(
				m,
				"sblock ring header: rxblk_addr :0x%0x, rxblk_rdptr :0x%0x, rxblk_wrptr :0x%0x, rxblk_size :%d, rxblk_count :%d, rxblk_blks: 0x%0x\n",
				ringhd_op->rx_addr, *(ringhd_op->rx_rd_p),
				*(ringhd_op->rx_wt_p), ringhd_op->rx_size,
				ringhd_op->rx_count, ringhd_op->rx_blks);
			seq_printf(
				m,
				"sblock ring header: txblk_addr :0x%0x, txblk_rdptr :0x%0x, txblk_wrptr :0x%0x, txblk_size :%d, txblk_count :%d, txblk_blks: 0x%0x\n",
				ringhd_op->tx_addr, *(ringhd_op->tx_rd_p),
				*(ringhd_op->tx_wt_p), ringhd_op->tx_size,
				ringhd_op->tx_count, ringhd_op->tx_blks);
			seq_printf(
				m,
				"sblock pool header: rxblk_addr :0x%0x, rxblk_rdptr :0x%0x, rxblk_wrptr :0x%0x, rxblk_size :%d, rxpool_count :%d, rxblk_blks: 0x%0x\n",
				poolhd_op->rx_addr, *(poolhd_op->rx_rd_p),
				*(poolhd_op->rx_wt_p), poolhd_op->rx_size,
				poolhd_op->rx_count, poolhd_op->rx_blks);
			seq_printf(
				m,
				"sblock pool header: txblk_addr :0x%0x, txblk_rdptr :0x%0x, txblk_wrptr :0x%0x, txblk_size :%d, txpool_count :%d, txblk_blks: 0x%0x\n",
...
int sblock_init_debugfs(void *root)
{
	if (!root)
		return -ENXIO;
	debugfs_create_file("sblock", 0444, (struct dentry *)root, NULL,
			    &sblock_debug_fops);
	return 0;
}
