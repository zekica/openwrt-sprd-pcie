...
static inline void writel_masked(void __iomem *addr, u32 clear_mask,
				 u32 set_mask)
{
	u32 val = readl(addr);
				    ...
				    enum qcom_pcie_event event)
{
	if (dev->event_reg && dev->event_reg->callback &&
		...
		pr_info("PCIe: Client of RC%d does not have registered for event %d.\n",
			dev->rc_idx, event);
	}
}
...
static void handle_wake_func(struct work_struct *work)
{
	...
	struct qcom_pcie *pcie =
		container_of(work, struct qcom_pcie, handle_wake_work);
...
static int qcom_pcie_get_resources_v3(struct qcom_pcie *pcie)
{
	...
	if (pcie->is_gen3) {
		res->axi_s_sticky_reset =
			devm_reset_control_get(dev, "axi_s_sticky");
...
static int qcom_pcie_init_v0(struct qcom_pcie *pcie)
{
	...
	writel(PCS_SWING_TX_SWING_FULL(0x78) | PCS_SWING_TX_SWING_LOW(0x78),
	       pcie->parf + PCIE20_PARF_PCS_SWING);
	writel(PHY_RX0_EQ(0x4), pcie->parf + PCIE20_PARF_CONFIG_BITS);

	/* Enable reference clock */
	writel_masked(pcie->parf + PCIE20_PARF_PHY_REFCLK, REF_USE_PAD,
		      REF_SSP_EN);
	...
	if (pcie->force_gen1) {
		writel_relaxed(
			((readl_relaxed(pcie->dbi +
					PCIE20_LNK_CONTROL2_LINK_STATUS2) &
			  (~PCIE_CAP_TARGET_LINK_SPEED_MASK)) |
			 SPEED_GEN1),
...
static int qcom_pcie_init_v2(struct qcom_pcie *pcie)
{
	...
	writel(MST_WAKEUP_EN | SLV_WAKEUP_EN | MSTR_ACLK_CGC_DIS |
		       SLV_ACLK_CGC_DIS | CORE_CLK_CGC_DIS | AUX_PWR_DET |
		       L23_CLK_RMV_DIS | L1_CLK_RMV_DIS,
	...
	writel(PCIE_CAP_CPL_TIMEOUT_DISABLE,
	       pcie->dbi + PCIE20_DEVICE_CONTROL2_STATUS2);
...
static int qcom_pcie_enable_resources_v3(struct qcom_pcie *pcie)
{
	...
	if (pcie->is_gen3) {
		...
		if (ret) {
			dev_err(dev,
				"cannot prepare/enable axi_bridge clock\n");
		...
		if (res->rchng_clk) {
			...
			if (ret) {
				dev_err(dev,
					"cannot prepare/enable rchng_clk clock\n");
				goto err_clk_rchng;
			}
...
static int qcom_pcie_init_v3(struct qcom_pcie *pcie)
{
	...
	if (pcie->is_gen3) {
		...
		writel(RXEQ_RGRDLESS_RXTS | GEN3_ZRXDC_NONCOMPL,
		       pcie->dbi + PCIE30_GEN3_RELATED_OFF);
	}
	...
	writel_masked(pcie->dbi + PCIE20_CAP_LINK_CAPABILITIES,
		      ...
		      (pcie->cap_active_state_link_pm & PCIE_ASPM_MASK)
			      << PCIE_ASPM_POS);
	...
	if (pcie->is_gen3) {
		for (i = 0; i < 255; i++)
			writel(0x0,
			       pcie->parf + PARF_BDF_TO_SID_TABLE + (4 * i));
		writel(0x4, pcie->dm_iatu + PCIE_ATU_CR1_OUTBOUND_6_GEN3);
		writel(0x90000000,
		       pcie->dm_iatu + PCIE_ATU_CR2_OUTBOUND_6_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_LOWER_BASE_OUTBOUND_6_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_UPPER_BASE_OUTBOUND_6_GEN3);
		writel(0x00107FFFF,
		       pcie->dm_iatu + PCIE_ATU_LIMIT_OUTBOUND_6_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_LOWER_TARGET_OUTBOUND_6_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_UPPER_TARGET_OUTBOUND_6_GEN3);
		writel(0x5, pcie->dm_iatu + PCIE_ATU_CR1_OUTBOUND_7_GEN3);
		writel(0x90000000,
		       pcie->dm_iatu + PCIE_ATU_CR2_OUTBOUND_7_GEN3);
		writel(0x200000,
		       pcie->dm_iatu + PCIE_ATU_LOWER_BASE_OUTBOUND_7_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_UPPER_BASE_OUTBOUND_7_GEN3);
		writel(0x7FFFFF,
		       pcie->dm_iatu + PCIE_ATU_LIMIT_OUTBOUND_7_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_LOWER_TARGET_OUTBOUND_7_GEN3);
		writel(0x0,
		       pcie->dm_iatu + PCIE_ATU_UPPER_TARGET_OUTBOUND_7_GEN3);
	}
...
static void handle_e911_func(struct work_struct *work)
{
	int slot_id;
	struct qcom_pcie *pcie =
		container_of(work, struct qcom_pcie, handle_e911_work);
...
static int pci_reboot_handler(struct notifier_block *this, unsigned long event,
			      void *ptr)
{
	pci_lock_rescan_remove();
	qcom_pcie_remove_bus();
...
static int qcom_pcie_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(pdev->dev.of_node, "qcom,pcie-ipq807x")) {
		...
		if (index < 0) {
			if (soc_version_major == 1) {
				pcie->phy = devm_phy_optional_get(
					dev, "pciephy-gen2");
				if (IS_ERR(pcie->phy))
	...
	pcie->mdm2ap_e911_irq = platform_get_irq_byname(pdev, "mdm2ap_e911");
	if (pcie->mdm2ap_e911_irq >= 0) {
		mdm2ap_e911 =
			devm_gpiod_get_optional(&pdev->dev, "e911", GPIOD_IN);
		...
		ret = devm_request_irq(&pdev->dev, pcie->mdm2ap_e911_irq,
				       ...
				       IRQ_TYPE_EDGE_BOTH, "mdm2ap_e911", pcie);
	...
	pcie->link_down_irq = platform_get_irq_byname(pdev, "int_link_down");
	...
	if (pcie->link_up_irq >= 0) {
		ret = devm_request_irq(&pdev->dev, pcie->link_up_irq,
				       handle_link_up_irq, IRQF_TRIGGER_RISING,
				       "pci_link_up", pcie);
	}
	...
	if (pcie->global_irq >= 0) {
		ret = devm_request_irq(&pdev->dev, pcie->global_irq,
				       ...
				       IRQF_TRIGGER_RISING, "qcom-pcie-global",
				       pcie);
		if (ret) {
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		...
		ret = devm_request_irq(dev, pp->msi_irq,
				       qcom_pcie_msi_irq_handler, IRQF_SHARED,
				       "qcom-pcie-msi", pp);
		if (ret) {
